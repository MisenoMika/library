...
void Gui::init() {
    ...
    if (has_colors()) {
        start_color();
        init_pair(1, COLOR_GREEN, COLOR_BLACK);   // 草
        init_pair(2, COLOR_WHITE, COLOR_BLACK);     // 兔子
        init_pair(3, COLOR_YELLOW, COLOR_BLACK);  // 狼
        init_pair(4, COLOR_RED, COLOR_BLACK); // 老虎（用品红代替橙色）
    }
}
...
//绘制实体
void Gui::drawSomething(WINDOW* win, int y, int x, char ch, int colorPair) {
